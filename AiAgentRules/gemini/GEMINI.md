...
- When adding a new enum in `__enums__.xlsx`, the `flags` column (C) must be set to `TRUE` if the enum is a bitmask, and `FALSE` otherwise.
- Entity `Data` classes (e.g., `BuffData.cs`) are not automatically generated by the `gen all` process. They need to be created manually.
- Entity `Data` classes (e.g., `WeaponData.cs`) are data-driven. They inherit from a base `EntityData` class and are initialized from a corresponding data table row class (e.g., `DRWeapon`) which is generated from an Excel file. The constructor takes an entity ID and a type ID, and uses the type ID to retrieve the data row from the corresponding data table.
- When creating new Excel files, delete the default 'Sheet1' as it is not needed for data generation.
- After adding a new Excel table, it must be registered in `g:\GameDevelopmentKitAI\Design\Excel\GameHot\Datas\__tables__.xlsx`.
- In Excel data tables, the first column (A) must contain literal strings like '##var', '##type', '##', not formulas. To write a literal string that starts with a special character using `write_data_to_excel`, prefix it with a single quote (e.g., `'##var`).
- 边进行工作边进行开发日志记录，并将确认后的新流程和细节记录到永久记忆中。
