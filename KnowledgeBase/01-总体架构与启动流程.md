...
| 根解决方案 | `Kit.sln` | 当前跟踪的 DotNet 与 Share 工具 IDE 入口 |
...
`Unity/Unity.sln` 是 Unity/IDE 可在本地生成的工程文件；当前 checkout 下该文件不存在，也不是 catalog 声明的仓库源，不应把它当作已跟踪入口维护。根 `Kit.sln` 包含 DotNet 与 Share 项目；`DotNet/DotNet.sln` 只包含服务端项目。
...
本次核验完成了源码静态链路与仓库路径检查，并静态确认当前 checkout 下 `Unity/Unity.sln` 不存在；同步到 UnityCode 时另行核对 Unity `6000.3.21f1`、UPM lock 更新和 ET Model 的 `ReactiveBinding` 引用变化，这些变更不改变本页启动主链。Unity 运行验证属于环境级回归，在修改启动流程时必须执行。
...
- `Kit.sln`：跟踪的 DotNet 与 Share 项目集合。
