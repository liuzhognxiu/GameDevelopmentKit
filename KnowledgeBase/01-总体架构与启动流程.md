...
`Unity/Unity.sln` 是 Unity 生成的 IDE 入口，不应手工维护；根 `Kit.sln` 包含 DotNet 与 Share 项目；`DotNet/DotNet.sln` 只包含服务端项目。
...
本次核验完成了源码静态链路与仓库路径检查；同步到 UnityCode 时另行核对 Unity `6000.3.21f1`、UPM lock 更新和 ET Model 的 `ReactiveBinding` 引用变化，这些变更不改变本页启动主链。Unity 运行验证属于环境级回归，在修改启动流程时必须执行。
