...
| `Unity/Unity.sln` | Unity 生成项目（**首次用 Unity 打开后自动生成**，不在仓库中） | 客户端、Loader、业务程序集与编辑器代码（不要手工维护） |
...
- `UNITY_ET` 与 `UNITY_GAMEHOT` 互斥；`UNITY_HOTFIX` 可叠加（业务转 DLL 资源）。热更新的启用/禁用行为详见[模块 20](20-热更新与一键打包.md)，此处不展开。
