...
本次核验完成了源码静态链路与仓库路径检查，并静态确认当前 checkout 下 `Unity/Unity.sln` 不存在；`Kit.sln` 新增 Buqi headless 验证项目、Luban active 状态切到 GameHot，都不改变本页描述的 Unity/服务端启动主链。Buqi 战斗内核仍缺独立知识模块，本轮只记录覆盖缺口；Unity 运行验证属于环境级回归，在修改启动流程时必须执行。
