...
5. 从 `GFEntityComponent` 调用 Child/Component Async API；移除 Child/Component 以关闭并 Dispose。
...
5. ET 模式创建一个 `UGFEntity<TView>`，检查 View/Transform 绑定、System 回调和 Dispose Hide。
6. 验证 Attach/Detach 的父子双方回调与 Transform 恢复。

本次已完成共享 Entity API、GameHot 实际玩法调用、ET UGFEntity/Mono 桥和 GFEntityComponent 创建入口的静态源码核验。运行和实例池行为需在 Unity 中回归。
