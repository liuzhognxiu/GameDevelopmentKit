...
5. `Unity/Assets/Tests/GameHot/Buqi/EditMode` 与 `Share/Buqi.Simulation.Headless` 构成 Buqi 战斗内核的确定性验证入口，并已由 `BUQI-01` 独立维护。新增正式测试时必须同时提交测试 asmdef/项目、执行命令和证据路径；Buqi 局部门禁不能冒充知识库五项通用运行验收。
...
- 本轮基于 `2253f0a2730b9793269b23011ea52acf6b572f3f` 的隔离 worktree 复现 4 个源码指纹漂移：`BUQI-01`、`UNITY-15`、`DATA-01`、`LIB-05`，来源是 Buqi effects/builds 扩展、GameHot Luban 表源、generated/bytes/json 输出、`TablesComponent` 接入和真实 generated bytes 测试更新。相关页面只记录当前源码可证实的局部事实，并保持生成目录禁止手改的规则。
- 本轮没有在隔离 worktree 执行 Unity 编译、Play Mode、服务端启动、Luban、Proto 或 Player 构建；`KnowledgeBase/runtime-acceptance.json` 保持 `not_run`。Buqi 主节点交接中的 headless 与 Unity 证据只作为 `BUQI-01` 局部门禁记录。
