...
5. 当前受 Git 跟踪的 `*Test*.cs` 主要是 ET Demo/工具示例，不构成统一自动化测试工程。新增正式测试时必须同时提交测试 asmdef/项目、执行命令和证据路径。
...
- 本轮开始时普通知识库门禁因当前工作区已有 9 个模块源码指纹漂移而失败；这些漂移来自本任务之外的 Unity/Luban/.NET/工具改动，本文不吸收或覆盖它们。
