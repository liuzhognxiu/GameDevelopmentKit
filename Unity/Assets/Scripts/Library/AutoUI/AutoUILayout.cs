...
namespace AutoUI
{
	public class AutoUILayoutProcessor
	{
		...
		public static void GridLayout参数自动推导(in Layer layer, ref GameObject parentGameObject)
		{
			...
			int column = 0;
			int row = 0;
			float paramSpacingX = -1f, paramSpacingY = -1f;
			float paramPadding = -1f;
			...
			foreach (var component in layer.components)
			{
				if (component.name == "grid")
				{
					...
					object o;
					if (component.parameters.TryGetValue("spacingX", out o)) paramSpacingX = Convert.ToSingle(o);
					if (component.parameters.TryGetValue("spacingY", out o)) paramSpacingY = Convert.ToSingle(o);
					if (component.parameters.TryGetValue("padding", out o)) paramPadding = Convert.ToSingle(o);
					break;
				}
			}
			...
			// 参数优先覆盖 spacing
			if (paramSpacingX >= 0f) spacingX = paramSpacingX;
			if (paramSpacingY >= 0f) spacingY = paramSpacingY;

			// 设置间距
			gridLayout.spacing = new Vector2(spacingX, spacingY);

			// 设置 padding
			float padding = paramPadding >= 0f ? paramPadding : AutoUIConfig.config.Default.Layout.Padding;
		...
		public static void ApplyVerticalLayout(in Layer layer, ref GameObject parent)
		{
			...
			// 参数优先
			var comp = layer.components?.FirstOrDefault(c => c.name == "verticalLayout");
			if (comp != null && comp.parameters != null && comp.parameters.Count > 0)
			{
				object o;
				if (comp.parameters.TryGetValue("spacing", out o)) layout.spacing = Convert.ToSingle(o);
				int padTop = 0, padBottom = 0, padLeft = 0, padRight = 0;
				if (comp.parameters.TryGetValue("paddingTop", out o)) padTop = Convert.ToInt32(o);
				if (comp.parameters.TryGetValue("paddingBottom", out o)) padBottom = Convert.ToInt32(o);
				if (comp.parameters.TryGetValue("paddingLeft", out o)) padLeft = Convert.ToInt32(o);
				if (comp.parameters.TryGetValue("paddingRight", out o)) padRight = Convert.ToInt32(o);
				layout.padding = new RectOffset(padLeft, padRight, padTop, padBottom);
				if (comp.parameters.TryGetValue("childControlWidth", out o)) layout.childControlWidth = Convert.ToBoolean(o);
				if (comp.parameters.TryGetValue("childControlHeight", out o)) layout.childControlHeight = Convert.ToBoolean(o);
				if (comp.parameters.TryGetValue("childForceExpandWidth", out o)) layout.childForceExpandWidth = Convert.ToBoolean(o);
				if (comp.parameters.TryGetValue("childForceExpandHeight", out o)) layout.childForceExpandHeight = Convert.ToBoolean(o);
				if (comp.parameters.TryGetValue("childAlignment", out o) && o is string)
				{
					TextAnchor anchor;
					if (System.Enum.TryParse((string)o, true, out anchor)) layout.childAlignment = anchor;
				}
				return;
			}
		...
		public static void ApplyHorizontalLayout(in Layer layer, ref GameObject parent)
		{
			...
			// 参数优先
			var comp = layer.components?.FirstOrDefault(c => c.name == "horizontalLayout");
			if (comp != null && comp.parameters != null && comp.parameters.Count > 0)
			{
				object o;
				if (comp.parameters.TryGetValue("spacing", out o)) layout.spacing = Convert.ToSingle(o);
