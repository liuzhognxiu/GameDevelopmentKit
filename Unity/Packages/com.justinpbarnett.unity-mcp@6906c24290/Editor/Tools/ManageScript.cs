    ...
    public static class ManageScript
    {
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            switch (action)
            {
                ...
                case "get":
                    return GetSelectedGoCom(name); // Alias for read
                default:
                    return Response.Error(
        ...
        private static object GetSelectedGoCom(string name)
        {
            ...
            {
                var selectedObject = selectedObjects[0]; // Get the first selected GameObject
                var scriptComponent = selectedObject.GetComponent(name);
                if (scriptComponent != null)
                {
                    MonoScript monoScript = MonoScript.FromMonoBehaviour(scriptComponent as MonoBehaviour);  

                    string scriptPath = AssetDatabase.GetAssetPath(monoScript);  
                    // If the component exists, return its type and name
                    return Response.Success(
                        $"Component '{name}' found on GameObject '{selectedObject.name}'.",
                        new { type = scriptComponent.GetType().FullName, name = selectedObject.name , path = scriptPath }
                    );
                }
                else
                {
                    return Response.Error(
                        $"Component '{name}' not found on GameObject '{selectedObject.name}'."
                    );
                }
            }
            else
            {
                return Response.Error("No GameObject selected.");
            }
        }
