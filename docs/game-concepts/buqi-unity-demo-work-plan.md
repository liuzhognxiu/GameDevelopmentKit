...
- 单机离线对手快照即可验证核心玩法，不需要 ET 房间、服务端或 MongoDB。
...
- 首个 Demo 不新增 Loader 装备；确有跨整个 GameHot 生命周期的服务时，再评审 `HotComponent`。
...
| 2 | 九装备战斗沙盒 | 日志、调试入口、9 装备/3 改造 | 不做正式 UI 也能验证三方向和 S/M/L 取舍 |
| 3 | 最小配置驱动 | Luban 战斗表与 9 装备/3 改造/6 对手快照 | 改 Excel 后导表即可改变验证内容 |
| 4 | 战斗可视化 | BattleForm、装备 Widget、回放 | 双方 8 格棋盘连续冷却自动战斗 |
| 5 | 构筑准备 | BoardForm、仓位、拖放/选择、单局随机状态 | 能购买、上阵、换位、合并和改造 |
| 6 | 三轮迷你局 | Run 状态、12 装备、3 商店、3 事件、9 对手快照 | 从开局玩到三场结算并继续调整 |
| 7 | 完整首阶段 Demo | 18 装备、6 事件、12 对手快照、完整胜负循环 | 8-12 分钟完成 5 胜场/3 单局生命局 |
...
6. 六类效果：伤害、护盾、加速、延迟、充能、过载。
7. 胜负：45 秒加时伤害、60 秒硬上限和平局。
...
- 快速方向：快速攻击（S）、充能攻击（M）、重型攻击（L）。
- 护盾反制：小型护盾（S）、破盾反击（S）、重型护盾（L）。
- 充能连锁：充能传递（S）、连锁攻击（S）、加速攻击（M）。

验证改造：高速、抗减速、复制。名称会在修仙包装统一时替换，规则保持不变。
...
- 三种方向各有一组真实联动可运行；护盾方向必须实际完成一次“获得护盾 -> 护盾受损 -> 转化反击”。
- 同一沙盒能验证 S/M/L 占位，并出现一次大型核心挤压辅助装备的取舍。
- 能解释每次伤害、护盾、延迟、充能和过载来源。
- 连续运行、关闭、重开不残留状态。
- Step 2 结束前不增加第七种效果。
...
把九装备验证内容从测试定义迁移到 GameHot Luban，运行时不依赖硬编码装备 ID 分支。本步骤只验证配置链路，不提前录入完整首阶段内容。
...
| 表 | 用途 |
|---|---|
| `BuqiGlobal.xlsx` | 生命值、护盾、过载、tick、加时伤害和基础规则 |
| `BuqiItem.xlsx` | 9 个验证装备、尺寸、价格、品质、冷却、标签和效果参数 |
| `BuqiRefinement.xlsx` | 3 种验证改造 |
| `BuqiEcho.xlsx` | 6 份教学/前期对手快照及完整构筑快照 |
...
- 9 装备、3 改造、6 对手快照数量校验通过；完整内容数量不属于本步骤验收。
- 生成目录无手工代码。
...
5. 显示双方 8 格、尺寸、生命值、护盾、过载、充能和连续冷却。
6. 提供暂停、1x/2x/4x、跳过、重播和日志过滤。
7. 高亮当前来源、目标、相邻连锁和延迟。
8. 战后展示伤害前 3、有效护盾前 2、关键 5 秒和事实摘要。
...
完成“购买 -> 上阵 -> 排列 -> 升级 -> 改造 -> 开战”的玩家操作闭环，并建立可保存、可复现的单局随机状态。

### 系统

- `RunState`：金币、胜场、单局生命、轮次、持有装备、临时效果和 `RunRandomState`。
- `RunRandomState`：保存 `RunSeed` 及准备入口、商店、事件奖励、对手快照选择四条命名随机流的消费游标。
- `BoardState`：8 格棋盘和 5 仓位。
- `BoardCommandService`：放置、交换、收回、出售、购买、合并、改造。
- 随机结果在生成时写入 `RunState`；界面预览、关闭重开和重复读取不得继续消费随机流。
- 所有修改通过命令返回成功/失败原因，UI 不直接改列表。

### UI

- `BuqiBoardForm`：棋盘、仓位、金币、胜场、单局生命和“确认开战”。
- `BuqiItemDetailForm`：完整装备文本、品质、尺寸、改造和预估冷却。
...
7. 在 Step 3 配置基础上增加起始方案、商店和事件表；扩至 12 个装备、全部 6 种改造、3 类商店、3 个事件和 9 份对手快照。
...
1. 全部 18 个装备和 6 种改造。
2. 全部 3 类商店、6 个事件、12 份对手快照。
3. 5 枚胜场胜利、3 点单局生命失败、最多 7 场有胜负战斗。
4. 对手快照筛选：胜场 -> 占位差 -> 构筑投入 -> 确定性选择。
5. 战前预览：方向、三个关键装备、主要威胁、占用和改造。
6. 平局重赛、连续平局补偿、对手快照版本拒绝和兜底。
...
3. 从 12 份基础对手快照按固定规则生成布局、品质和改造变体；按相同投入档执行完整交叉对战并交换左右阵营，每个唯一向量只计一次。
4. 按投入档和方向等权汇总胜率；确定性重复运行只用于 hash 验证，不计入平衡样本量。
5. 另用固定生成器创建 10,000 份不同合法构筑，检查异常、循环上限和性能，不混入胜率。
6. 检查采用率 >75% 的装备和贡献提升 >50% 且代价未发生的改造。
