...
| 字段 | 定义 |
|---|---|
| Purpose | 连续制造“读取题目—押注—验证—订正”，直到取得足够胜场或失去全部单局生命 |
| Player experience goal | 每轮都能回答“我为什么这样改，以及下一战预期哪里不同” |
| Owner | `BuqiRunController` + 纯 C# `RunState` |
| Inputs | 起始方案、准备候选、玩家命令、对手快照摘要、BattleResult |
...
1. 创建版本化 `RunState`。
2. 写入 `[PLACEHOLDER]` 初始金币 6、`[PLACEHOLDER]` 单局生命 3、胜场 0。
3. 固化三套起始方案候选；玩家只能选择一套。
4. 生成第 1 轮对手快照与准备入口，不因查看详情重新生成。

**Rationale**：三套起始方案用于教学三种关系，不是流派承诺。初始资源必须让玩家在第一轮做一次购买或保留的真实取舍；具体 6 金币待 P-1/迷你局验证。
...
| EchoIntel | 判断主要威胁 | 已固化对手快照摘要 | 战前意图草稿 | 查看后更换对手 |
| PreparationA/B | 投资、转向或保留 | 固化入口与资源 | StateDelta | UI 重开重抽 |
| BoardReview | 位置、尺寸、仓位取舍 | 持有装备 | 合法快照 | 自动替玩家挤位置 |
...
| RoundSettlement | 接受胜场/单局生命与奖励 | 结果、未发放标记 | 下一轮或终局 | 重复结算 |

## 4. 进度与终局

- `[PLACEHOLDER]` 获得 5 枚胜场：RunVictory。
- `[PLACEHOLDER]` 单局生命从 3 降至 0：RunDefeat。
- 每场有胜负战斗：胜利 +1 胜场；失败 -1 单局生命。
- `[DERIVED]` 最多 7 场有胜负战斗，是当前 5 胜/3 败组合的数学上界，不含平局；上游终局变量变化时重新计算。
...
- 战斗请求 hash 已有结果时恢复该结果，不重新选择对手快照。
...
| 棋盘无合法装备 | 禁止确认，定位具体非法状态 |
| 战斗后崩溃 | 从已保存 BattleResult 恢复，结算一次 |
| 平局后换对手快照 | 不重新准备；固化新对手快照并写入状态 |
...
| 胜场/单局生命 | `[PLACEHOLDER]` 5/3 | 目标约 4–7 次闭环；测局长与逆转率 |
...
- 相同 RunSeed 与操作序列产生相同候选、对手快照和结果引用。
- UI 重开不改变状态与 RNG 游标。
- 所有结算幂等。
- 非法阶段命令返回明确原因，不静默执行。
