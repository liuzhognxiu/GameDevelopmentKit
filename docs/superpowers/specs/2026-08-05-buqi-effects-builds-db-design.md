...
The first implemented catalog uses 8 builds, 24 items, 6 refinements, and 16 echoes under `ContentVersion=buqi-effects-cv1`. The deterministic core, Luban schema/data, generated bytes, validator, Unity EditMode tests, and headless contracts are implemented; formal player-facing enablement remains gated by P-1.
