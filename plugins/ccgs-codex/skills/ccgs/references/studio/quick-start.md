...
Templates are in `references/studio/templates/`:
...
Also in `references/studio/templates/collaborative-protocols/` (used by agents, not typically edited directly):
...
2. **Set up the engine** — Run `/setup-engine` (uses the brainstorm recommendation)
   ...
   - Creates `references/studio/technical-preferences.md` with naming conventions,
