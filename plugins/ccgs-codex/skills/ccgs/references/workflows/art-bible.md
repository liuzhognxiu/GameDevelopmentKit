...
Read `references/studio/technical-preferences.md` if it exists — extract performance budgets and engine for asset standard constraints.
...
- **`technical-artist`**: Engine-specific hard constraints — poly count budgets per asset category, texture memory limits, material slot counts, importer constraints, anything from the performance budgets in `references/studio/technical-preferences.md`
...
After all sections are complete (or the scoped set from Phase 1 is complete), spawn `creative-director` via Task using gate **AD-ART-BIBLE** (`references/studio/director-gates.md`).
...
- Does `references/studio/technical-preferences.md` contain a configured engine (not `[TO BE CONFIGURED]`)? → setup-engine is done, skip that option
